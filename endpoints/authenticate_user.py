...
def authenticate_user(connection, data):
    ...
    user_data = {user[1]: {'userId': user[0], 'password': user[2], 'avatar': user[4]} for user in users}
    ...
    if password == stored_password:
        return jsonify({"loggedIn": True, "username": username, "userId": user_data[username]['userId'], "avatar": user_data[username]['avatar']}), 200
    else:
